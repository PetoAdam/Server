...
namespace DummyServer
{
    class ServerHandle
    {
        ...
        public static void OnPlayerMovement(int _fromClient, Packet _packet)
        {
            ...
            if(m != null)
            {
                ServerSend.OnPlayerMovement(m.host.id, _inputs, _rotation, _username, _xRot, _camForward);
            }

            Match m2 = Server.matchDatabase.GetMatchByPlayer(Server.playerDatabase.GetPlayerById(_fromClient));
            if (m2 != null)
            {
                ServerSend.OnPlayerMovement(m2.host.id, _inputs, _rotation, _username, _xRot, _camForward);
            }
